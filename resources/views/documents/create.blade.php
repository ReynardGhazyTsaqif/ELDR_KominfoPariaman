    ...
    <div class="max-w-4xl mx-auto py-2">
        ...
        <div class="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100/80">
            ...
            <form action="{{ route('documents.store') }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-6">
                ...
                <!-- Row 2: UPLOAD FILE Drag & Drop Box with Live Visual Feedback -->
                <div>
                    <label class="block text-xs font-extrabold text-gray-700 uppercase tracking-wider mb-2">
                        UPLOAD FILE BERKAS (.DOC / .DOCX) <span class="text-rose-500">*</span>
                    </label>
                    
                    <div id="upload-box-container" class="relative border-2 border-dashed rounded-2xl p-8 text-center cursor-pointer transition-all border-gray-300 bg-gray-50/50 hover:bg-gray-50 hover:border-gray-400">
                        <input type="file" name="file_dokumen" id="file_dokumen" accept=".doc,.docx" required class="absolute inset-0 w-full h-full opacity-0 cursor-pointer" onchange="handleFileSelected(this)">
                        ...
                        <div id="upload-default-prompt" class="flex flex-col items-center justify-center gap-2">
                            ...
                            <h4 class="text-sm font-bold text-gray-800">
                                Klik untuk unggah atau seret file Word ke sini
                            </h4>
                        ...
                        <!-- Feedback State (File chosen) -->
                        <div id="upload-success-prompt" class="hidden flex flex-col items-center justify-center gap-2 py-2">
                            <div class="w-12 h-12 rounded-full bg-emerald-100 text-emerald-600 flex items-center justify-center mb-1 shadow-xs">
                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7" />
                                </svg>
                            </div>
                            <span class="inline-flex items-center gap-1.5 bg-emerald-100 text-emerald-800 font-extrabold px-3.5 py-1 rounded-full text-xs uppercase tracking-wider shadow-xs">
                                ✓ File Berhasil Dipilih
                            </span>
                            <h4 id="display-filename" class="text-base font-extrabold text-emerald-900 break-all max-w-md mt-1"></h4>
                            <p id="display-filesize" class="text-xs text-emerald-700 font-bold"></p>
                            <p class="text-[11px] text-gray-500 font-medium underline mt-2 hover:text-gray-700">Klik di mana saja pada kotak ini jika ingin mengganti berkas</p>
                        </div>
                    </div>
                </div>
